...
func enableWebhook(mgr manager.Manager, validate config.Validate, namespace string, logger log.Logger) error {
	...
	if err := (&metallbv1beta1.AWSAdvertisement{}).SetupWebhookWithManager(mgr); err != nil {
		level.Error(logger).Log("op", "startup", "error", err, "msg", "unable to create webhook", "webhook", "AWSAdvertisement")
		return err
	}
